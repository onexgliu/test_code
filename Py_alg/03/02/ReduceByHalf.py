def search(data,num):#定义查找函数，参数是原数列data和键值num
    ...
    while low<=high and num!=-1:
        ...
        if num<data[mid]:#判断数据小于中间值
            print('%d 介于故障点位置%d[%d]和中间故障点位置%d[%d]之间，找左半边'%(num,low+1,data[low],mid+1,data[mid]))#输出位置再数列中的左半边
            high=mid-1                          #最高位变成了中间位置减1
        elif num>data[mid]: #判断数据大于中间值
            print('%d 介于中间故障点位置%d[%d]和故障点位置%d[%d]之间，找右半边'%(num,mid+1,data[mid],high+1,data[high]))#输出位置再数列中的右半边
